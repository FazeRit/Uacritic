...
const Card: FC<{ item: CardProps["item"], category: string }> = ({ item, category }) => {
    return (
        <div className="flex flex-col w-[19.5rem] bg-primaryText text-white min-h-[19rem] rounded-2xl transform transition-transform duration-300 hover:scale-110">
            <CardImg title={item.title} category={category} value={item.value} imageUrl={item.imageUrl}/>
            <div className="flex flex-row mx-3 mt-2 min-h-10">
                {/*TODO: Make Link path */}
                <Link href="/" className="text-lg roboto-medium w-[9rem] break-words">{item.title.length <= 20 ? item.title: item.title.slice(0, 12) + '...'}</Link>
                <CardRate rate={item.rate}/>
                <p className="ml-[2.8rem]">
